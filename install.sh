...
for file in ~/.bash_aliases ~/.bash_profile ~/.bashrc ~/.config/fish/config.fish ~/.vimrc ~/.nanorc ~/.tmux.conf ~/.config/hypr ~/.config/waybar ~/.config/wofi ~/.config/kitty ~/.config/wlogout ~/.zshrc ~/.config/nvim ~/.config/ghostty; do
  if [ -e "$file" ]; then
...
stow -v bash fish vim nano tmux hypr waybar wofi kitty wlogout zsh nvim ghostty
